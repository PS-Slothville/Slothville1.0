...
function TornadoParticle:OnUpdate(gameTime)
    ...
    if not sentSync then 
        sentSync = true
        print("Syncing Tornado")
        globalCoords = { x = vortex.x + offset.x, y= vortex.y + offset.y, z = vortex.z + offset.z }
        TriggerServerEvent("updateCoords", vortex.x + offset.x, vortex.y + offset.y, vortex.z + offset.z)
        Citizen.SetTimeout(10000, function()
            print("This will be sent after 10 seconds.")
            sentSync = false
        end)
    end
end
