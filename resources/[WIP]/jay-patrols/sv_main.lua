...
local function Spawn(source, type, model, coords, route)
    local src = source
    -- print(coords)
    ...
    TriggerClientEvent('jay-patrols:server:catchGuard', src, netId, route)
end exports("Spawn", Spawn)

-- RegisterNetEvent('jay-patrols:server:spawnGuard', function(type, model, coords, route)
--     local src = source
--     Guard = CreatePed(type, model, coords.x, coords.y, coords.z, coords.w, true, false)
--     while not DoesEntityExist(Guard) do Wait(0) end
